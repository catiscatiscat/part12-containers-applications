...
io.on('connection', function (socket) {
  const welcomeMsg = 'Welcome to socketIo server';

  socket.emit('welcome to server', welcomeMsg);

  socket.broadcast.emit('player logged in');

  socket.on('event://send-newplayer', function (msg) {
    console.log('back new player');
    console.log('got newPlayer', msg);
    const message = JSON.parse(msg);
    console.log('rooms:');
    console.log(rooms);
    if (rooms[message.roomId] && !rooms[message.roomId].gameOn) {
      const payload = {
